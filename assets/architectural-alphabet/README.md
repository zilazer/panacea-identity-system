# PANACEA — Architectural Alphabet

《PANACEA — Architectural Alphabet》是一套将 Panacea Architectural Regular 字形与建筑原型逐字对应的视觉研究。

This series maps every glyph in Panacea Architectural Regular to an architectural archetype.

## Series constants / 系列统一规则

- Winter, 10:00 AM / 冬季上午 10 点
- Dense fog, no people / 大雾、无人
- Concrete industrial architecture / 混凝土工业建筑
- Wet ground / 湿地面
- Telephoto compression / 长焦压缩
- Cool neutral gray palette / 冷灰色调
- Systematic frontal architectural photography / 客观、正立面的建筑类型学摄影
- Minimal composition, large overcast sky / 极简构图、大面积阴天天空
- Architecture occupies about 80% of the visual subject / 建筑主体约占画面 80%
- No logos, vehicles, signage, text, or plant subject / 无 logo、车辆、标识、文字或植物主体

## Mapping / 对应关系

| Glyph | Archetype | 中文 |
| --- | --- | --- |
| a | Single portal | 单门厂房 |
| b | Warehouse with elevator shaft | 外挂电梯井仓库 |
| c | Open L-shaped wall | L 型折墙 |
| d | Factory with vertical core | 核心筒厂房 |
| e | Cantilever slab | 混凝土挑板 |
| f | Industrial canopy | 工业雨棚 |
| g | Factory with underground ramp | 地下坡道厂房 |
| h | Double-column colonnade | 双柱柱廊 |
| i | Concrete tower | 混凝土塔 |
| j | Bent signal tower | 折塔 |
| k | Industrial fire stair | 工业消防楼梯 |
| l | Concrete corner | 厂房转角 |
| m | Three-bay factory | 三跨厂房 |
| n | Two-bay factory | 双跨厂房 |
| o | Square courtyard | 方形天井 |
| p | Ramp warehouse | 坡道仓库 |
| q | Sunken courtyard | 地下庭院 |
| r | Gatehouse | 门卫室 |
| s | Continuous folded wall | 连续折墙 |
| t | Landmark tower with crossbeam | 横梁地标塔 |
| u | Three-sided courtyard | 三面围合庭院 |
| v | Concrete canyon | 混凝土峡谷 |
| w | Four-bay colonnade group | 四跨柱廊群 |
| x | Crossed structural joint | 交叉结构节点 |
| y | Transport tower and bridge | 运输塔与运输桥 |
| z | Switchback ramp and roof | 折返坡道与屋顶 |

## Output folders / 输出目录

- `photos/`: architectural photographs / 建筑摄影原片
- `../../panacea.html`: 26 responsive grid-glyph + photograph spreads / 26 组响应式字母网格与摄影双页
